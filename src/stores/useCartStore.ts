...
const useCartStore = create((set, get) => ({
  ...
  addToCart: ({
    ...
    try {
      ...
      const itemExists = currentItemsList.findIndex((itemInList: any) =>
        objetosIguales(itemInList.data, newItemData)
      );
      if (itemExists !== -1) {
  ...
  setQuantity: (itemId: any, quantity: Number) => {
    ...
    if (searchedItemIndex !== -1) {
      ...
      // Recalcula el precio total del item
      currentItemsList[searchedItemIndex].data.priceData.totalPrice =
        currentItemsList[searchedItemIndex].data.priceData.unitPrice * quantity;
      set({ items: currentItemsList });
      get().calculateItemsCount();
      get().calculateTotalPrice();
    }
  ...
  calculateItemsCount: () => {
    const currentItemsList = get().items;
    const itemsCount = currentItemsList.reduce(
  ...
  calculateTotalPrice: () => {
    const currentItemsList = get().items;
    const totalPrice = currentItemsList.reduce(
      (total: any, item: any) => total + item.data.priceData.totalPrice,
      0
    );
    console.log("Precio recaculado: ", totalPrice);
  ...
  getCartData: () => {
    ...
    return {
      ...
      items: currentItemsList.map((item: any) => ({
        ...
        variantId: item.data.selectedVariant
          ? item.data.selectedVariant.id
          : null,
